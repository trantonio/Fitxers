...
//						}
			...
			for(int i =0; i<Accidentes.accidentes.size();i++){
				if(!oldName.equals(Accidentes.accidentes.get(i).getNombre())){
					...
					writer.close();
						//He creado otro array a 0 para cambiar los valores a 0
						for(int j = 0; j<cont.length ;j++){
							for (int h = 0; h <cont[j].length; h++) {
								cont[j][h]= 0;
							}
...
//					System.out.println(logFile.getCanonicalPath());
					...
					if(Accidentes.accidentes.get(i).getCausa().equals(cause)) {
						cont[Accidentes.accidentes.get(i).getHora()][dayOfWeek(Accidentes.accidentes.get(i).getDiaSem()) - 1]++;
						// Comprovar lo que hacia la version 1
//						writer.write(Accidentes.accidentes.get(i).getNombre() + '\t' + dayOfWeek(Accidentes.accidentes.get(i).getDiaSem()) + '\t' + Accidentes.accidentes.get(i).getHora() + '\t' + Accidentes.accidentes.get(i).getCausa() + '\n');
					}
				}
			...
			//No hace falta cerrar porque tenemos un try con recursos
			writer.close();
